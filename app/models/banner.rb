class Banner < ApplicationRecord
  ...
  scope :with_active,   -> { where("post_started_at <= ?", Date.current).where("post_ended_at >= ?", Date.current) }

  scope :with_inactive, -> { where("post_started_at > ? or post_ended_at < ?", Date.current, Date.current) }
